...
int main(void)
{
  ...
  const char *path_tokens[] = {"run", "root", "ping"};
  ...
  if (strcmp(c->entrypoint, "run") != 0 || strcmp(c->topic, "root") != 0) {
    fprintf(stderr, "catalog_smoke: taxonomy mismatch for yai.root.ping\n");
    yai_sdk_command_catalog_free(&cat);
    return 4;
  }

  c = yai_sdk_command_catalog_find_by_canonical_path(&cat, "run root ping", NULL);
  if (!c) {
    fprintf(stderr, "catalog_smoke: expected canonical path run root ping\n");
    yai_sdk_command_catalog_free(&cat);
    return 5;
  }
